...
static size_t
client_id_rq_cb (void *cls, size_t size, void *buf);
/****************************************************************************************/
...
size_t
p2p_transmit_ready_notify
(void *cls, size_t size, void *buf)
{
	struct PeerContext *p = cls;
	struct GNUNET_EXT_P2P_PUT my_msg;
	size_t msg_size = sizeof(struct GNUNET_EXT_P2P_PUT);
	my_msg.header.size = htons((uint16_t) msg_size);
	my_msg.header.type = htons(GNUNET_MESSAGE_TYPE_EXT_P2P_PUT);
	my_msg.stripe = htonl(rand() % 100);
...
static size_t
client_create_rq_cb (void *cls, size_t size, void *buf)
{
	struct GNUNET_SCRB_ServiceReplyCreate *hdr = cls;

	size_t msg_size = sizeof(struct GNUNET_SCRB_ServiceReplyCreate);
	hdr->header.size = htons((uint16_t) msg_size);
	hdr->header.type = htons(GNUNET_MESSAGE_TYPE_SCRB_CREATE_REPLY);

	if(NULL == buf){/* handle error here */; return 0;}

	GNUNET_assert(size >= msg_size);

	memcpy(buf, hdr, msg_size);

	return msg_size;
}

static size_t
client_subscribe_rq_cb (void *cls, size_t size, void *buf)
{
	struct GNUNET_SCRB_ServiceReplySubscribe hdr;
	struct GNUNET_SCRB_ServiceSubscriber* sub = cls;

	size_t msg_size = sizeof(struct GNUNET_SCRB_ServiceReplySubscribe);
	hdr.header.size = htons((uint16_t) msg_size);
	hdr.header.type = htons(GNUNET_MESSAGE_TYPE_SCRB_SUBSCRIBE_REPLY);
	hdr.cid = sub->cid;
	hdr.group_id = sub->group_id;
...
static int
remove_subscriber (void *cls,
		const struct GNUNET_HashCode *key,
		void *value)
{
	struct GNUNET_SCRB_ServiceSubscriber *sub = value;
	struct GNUNET_SCRB_ServiceReplyLeave* mes = cls;

	if (0 == memcmp(&sub->group_id, &mes->group_id, sizeof(struct GNUNET_HashCode))) {
		GNUNET_free(sub);
	}
	return GNUNET_OK;
}
		...
		const struct GNUNET_MessageHeader *message)
{
	...
	if (NULL ==
			GNUNET_SERVER_notify_transmit_ready (ce->client,
					sizeof (struct GNUNET_SCRB_ServiceReplyCreate),
					GNUNET_TIME_UNIT_FOREVER_REL,
					&client_create_rq_cb, hdr))
		GNUNET_SERVER_receive_done (ce->client, GNUNET_SYSERR);
		...
		const struct GNUNET_MessageHeader *message)
{
	...
	if (NULL ==
			GNUNET_SERVER_notify_transmit_ready (ce->client,
					sizeof (struct GNUNET_SCRB_ServiceReplySubscribe),
					GNUNET_TIME_UNIT_FOREVER_REL,
					&client_subscribe_rq_cb, sub))
		GNUNET_SERVER_receive_done (ce->client, GNUNET_SYSERR);
		...
		const struct GNUNET_MessageHeader *message)
{
	...
	GNUNET_DHT_put (dht_handle, &hdr->group_id, 10U,
			GNUNET_DHT_RO_RECORD_ROUTE |
			GNUNET_DHT_RO_DEMULTIPLEX_EVERYWHERE | GNUNET_DHT_RO_LAST_HOP,
			GNUNET_BLOCK_SCRB_TYPE_MULTICAST,
			sizeof (multicast_block), &multicast_block,
		...
		const struct GNUNET_MessageHeader *message)
{
	...
	GNUNET_DHT_put (dht_handle, &leave_block.group_id, 10U,
			GNUNET_DHT_RO_RECORD_ROUTE |
			GNUNET_DHT_RO_DEMULTIPLEX_EVERYWHERE | GNUNET_DHT_RO_LAST_HOP,
		...
		const struct GNUNET_MessageHeader *message)
{
	...
	GNUNET_DHT_put (dht_handle, &group_id, 10U,
			GNUNET_DHT_RO_RECORD_ROUTE |
			GNUNET_DHT_RO_DEMULTIPLEX_EVERYWHERE | GNUNET_DHT_RO_LAST_HOP,
		...
		const struct GNUNET_MessageHeader *message)
{
	if (NULL ==
			GNUNET_SERVER_notify_transmit_ready (client,
					sizeof (struct GNUNET_SCRB_ServiceReplyIdentity),
					GNUNET_TIME_UNIT_FOREVER_REL,
					&client_id_rq_cb, client))
		...
		const struct GNUNET_CONFIGURATION_Handle *c)
{
	static const struct GNUNET_SERVER_MessageHandler handlers[] = {
			{&handle_cl_id_request, NULL, GNUNET_MESSAGE_TYPE_SCRB_ID_REQUEST, sizeof(struct GNUNET_SCRB_ClientRequestIdentity)},
